...
const Details: React.FC<{ poll: PollStruct }> = ({ poll }) => {
 ...
 // Update current time every second to ensure isEnded updates in real-time
  ...
  const isEnded = React.useMemo(() => {
    // Handle both string and number formats for endsAt
    const endTime = typeof poll.endsAt === 'string' 
        ...
        <div className=" h-[136px] gap-[16px] flex flex-col items-center mt-4">
          ...
          {wallet && (
            <div className="flex flex-col items-center gap-2">
              <p className="text-xs uppercase tracking-wider text-[#B0BAC9]">Connected Wallet</p>
              <div
                className="flex items-center justify-center min-w-[133px] h-[32px]
                     py-[20px] px-3 rounded-[10px] gap-[12px] bg-black/30"
              >
                <div className="w-[32px] h-[32px] rounded-full bg-[#1B5CFE]" />
                <p className="text-[12px] md:text-[14px] font-[500px] break-all">
                  {wallet}
                </p>
              </div>
            </div>
          ...
          <div className="h-[36px] gap-[4px] flex justify-center items-center">
            ...
            {wallet && wallet.toLowerCase() === poll.director?.toLowerCase() && poll.votes < 1 && (
              <button
                className="py-[6px] px-[12px] 
            ...
            {wallet && wallet.toLowerCase() === poll.director?.toLowerCase() && (poll.votes < 1 || isEnded) && (
              <button
                className="py-[6px] px-[12px] 
              border border-gray-400 bg-white bg-opacity-20 rounded-full 
              text-[12px] md:text-[16px] gap-[8px] flex justify-center items-center"
