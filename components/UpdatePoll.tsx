...
const UpdatePoll: React.FC<{ pollData: PollStruct }> = ({ pollData }) => {
  ...
  const handleUpdate = async (e: FormEvent) => {
    ...
    try {
      ...
      // Wait a moment for blockchain state to update, then reload the page
      // This ensures all data is fresh including images
      setTimeout(() => {
        if (router.pathname === '/') {
          // If on home page, just refresh the data
          router.replace(router.asPath)
        } else {
          // If on poll detail page, reload to get fresh data
